...
> 이문제는 dp로 풀면 풀리지 않는 문제이다. 정확히는 dp[][][][][][][][][x] [y] [k] 로 풀면 풀리지 않고 BFS로 풀어야하는 문제이다.
...
> 위 반례를 보면 알 수 있다. 
>
> 출발지점에서 아래쪽으로 먼저 가는 경우 (4, 4) 에 도착했을 때, (3, 4) 로 이동한다면 (3, 4) 의 dp에 들어가는 값은 -1이 된다. 아직 (4, 4)의 dp에는 최소값이 들어있지 않기 때문이다. 그 이후 출발지점에서 오른쪽으로 가는 경우에서 (3, 4) 에 도착한다면 dp 에 -1이 들어있기 때문에 8이 출력된다.
>
> 답은 6이다.